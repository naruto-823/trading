...
口径：所有 `pct` 字段以**账户净资产（含现金）**为分母。账户快照缺失时
回退到持仓总市值，保证空数据也能跑。
"""
...
# 集中度阈值（净资产口径，用于生成 alerts，纯描述性，不构成建议）
...
def _to_hkd(value: float, currency: str, db: Session | None = None) -> float:
    """把金额换算成 HKD（走 fx 服务拿真实汇率）"""
    return fx_service.to_hkd(value, currency, db)
...
def analyze_portfolio(db: Session) -> dict[str, Any]:
    ...
    for p in all_positions:
        ...
        mv_hkd = _to_hkd(abs(p.market_value), p.currency, db)
        cost_hkd = _to_hkd(abs(p.cost_price * p.quantity) * (100 if is_option else 1), p.currency, db)
        pnl_hkd = _to_hkd(p.unrealized_pnl, p.currency, db)
        day_pnl_hkd = _to_hkd(p.day_pnl, p.currency, db)
    ...
    # pct 分母：优先净资产（含现金），缺账户快照时回退到持仓总市值
    account = get_latest_account(db)
    pct_denom_hkd = (
        float(account.net_assets) if account and account.net_assets > 0 else total_mv_hkd
    )
    pct_basis = "net_assets" if account and account.net_assets > 0 else "market_value"

    return {
        "summary": _build_summary(items, total_mv_hkd, pct_denom_hkd, pct_basis),
        "concentration": _build_concentration(items, pct_denom_hkd),
        "pnl_distribution": _build_pnl_distribution(items),
        "cost_structure": _build_cost_structure(items, total_mv_hkd, pct_denom_hkd),
        "derivatives": _build_derivatives(items, pct_denom_hkd),
        "alerts": _build_alerts(items, pct_denom_hkd, pct_basis),
        "disclaimer": (
            "本分析仅基于账户当前持仓数据进行客观统计描述，"
            "所有 pct 字段以净资产（含现金）为分母；"
...
def _build_summary(
    items: list[dict], total_mv_hkd: float, pct_denom_hkd: float, pct_basis: str
) -> dict[str, Any]:
    ...
    return {
        ...
        "net_assets_hkd": round(pct_denom_hkd, 2),
        "pct_basis": pct_basis,  # net_assets / market_value（回退）
        "currency_breakdown": [
            {"currency": cur, "market_value_hkd": round(mv, 2), "pct": _safe_pct(mv, pct_denom_hkd)}
            for cur, mv in sorted(by_currency.items(), key=lambda x: -x[1])
        ],
        "market_breakdown": [
            {"market": mk, "market_value_hkd": round(mv, 2), "pct": _safe_pct(mv, pct_denom_hkd)}
            for mk, mv in sorted(by_market.items(), key=lambda x: -x[1])
        ],
    }


def _build_concentration(items: list[dict], pct_denom_hkd: float) -> dict[str, Any]:
    ...
    top_holdings = [
        {
            ...
            "pct": _safe_pct(i["market_value_hkd"], pct_denom_hkd),
        }
...
def _build_cost_structure(
    items: list[dict], total_mv_hkd: float, pct_denom_hkd: float
) -> dict[str, Any]:
    """成本 vs 现值组成。

    - `cost_pct`：单仓成本 / 持仓总成本（持仓内部组成）
    - `market_value_pct`：单仓市值 / 净资产（统一对外口径）
    - `weight_drift_pct`：mv_pct_in_holdings − cost_pct，反映"在持仓里的相对权重漂移"，
      所以内部仍用持仓基底算 mv_pct，避免被现金摊薄而失去组成对比意义。
    """
    total_cost_hkd = sum(i["cost_value_hkd"] for i in items) or 0.0
    structure = []
    for i in items:
        cost_pct = _safe_pct(i["cost_value_hkd"], total_cost_hkd)
        mv_pct_assets = _safe_pct(i["market_value_hkd"], pct_denom_hkd)
        mv_pct_holdings = _safe_pct(i["market_value_hkd"], total_mv_hkd)
        structure.append({
            "symbol": i["symbol"],
            "name": i["name"],
            "cost_pct": cost_pct,
            "market_value_pct": mv_pct_assets,
            "weight_drift_pct": round(mv_pct_holdings - cost_pct, 2),  # 正：涨出来的权重
        })
    return {
...
def _build_derivatives(items: list[dict], pct_denom_hkd: float) -> dict[str, Any]:
    ...
    return {
        ...
        "option_pct": _safe_pct(option_mv, pct_denom_hkd),
...
def _build_alerts(
    items: list[dict], pct_denom_hkd: float, pct_basis: str
) -> list[dict[str, str]]:
    """生成客观风险描述（不是建议，只是数字达到阈值时的提示）"""
    alerts: list[dict[str, str]] = []
    if not items or pct_denom_hkd <= 0:
        return alerts
    basis_label = "净资产" if pct_basis == "net_assets" else "持仓总市值"

    sorted_items = sorted(items, key=lambda x: -x["market_value_hkd"])
    top1_pct = _safe_pct(sorted_items[0]["market_value_hkd"], pct_denom_hkd)
    if top1_pct >= SINGLE_POSITION_HIGH_PCT:
        alerts.append({
            "level": "info",
            "type": "single_concentration",
            "message": f"{sorted_items[0]['symbol']} 占{basis_label} {top1_pct}%，超过 {SINGLE_POSITION_HIGH_PCT}% 阈值",
        })

    top3_pct = sum(_safe_pct(i["market_value_hkd"], pct_denom_hkd) for i in sorted_items[:3])
    if top3_pct >= TOP3_HIGH_PCT:
        alerts.append({
            "level": "info",
            "type": "top3_concentration",
            "message": f"前三大持仓合计占{basis_label} {round(top3_pct, 2)}%，超过 {TOP3_HIGH_PCT}% 阈值",
        })
    ...
    for cur, mv in by_currency.items():
        pct = _safe_pct(mv, pct_denom_hkd)
        if pct >= 80.0:
            alerts.append({
                "level": "info",
                "type": "currency_concentration",
                "message": f"{cur} 资产占{basis_label} {pct}%，外汇敞口集中",
            })
