...
- **近期宏观快讯_中文**：金十/财联社/华尔街见闻的实时快讯，**多数是中文**。importance 1-5（4-5 是源自己标记的"重要新闻"）：
  - 优先看 importance>=4 的（FOMC、CPI、关键数据、地缘升级、央行政策等）
  - importance=2 的快讯需要你自己判断是否对用户持仓有传导效应
  - 这条流是 **macro 事件的主要数据源**——symbol=null 的 events 主要从这里来"""
...
def detect_events(db: Session) -> list[dict]:
    ...
    macro_items = fetch_macro_news(min_importance=2, hours_back=4)
