...
【输入字段说明】
- **近期新闻_按持仓**：每只重仓股 Tier1/2/3 来源的新闻；summary 是正文摘要（Finnhub/Tavily 提供，比 title 信息更全）。**有 summary 时优先基于 summary 判断**，避免单凭标题误判。source_tier=finnhub 是专业财经源；google_rss 是兜底，质量参差，要更严格
- **近期宏观快讯_中文**：金十/财联社/华尔街见闻的实时快讯，**多数是中文**，importance=2（源未标"重要"的普通快讯）：
...
def detect_events(db: Session) -> list[dict]:
    ...
    # 都没有数据才跳过；只要任一源有内容就值得调 LLM 一次
    if all(not v for v in news_by_symbol.values()) and not macro_items:
        return []
