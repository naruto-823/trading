...
from app.services.macro_feed import fetch_macro_news
from app.services.mcp_jin10 import jin10_session, search_for_position
...
【输入字段说明】
- **近期新闻_按持仓_英文源**：每只重仓股 Finnhub/Tavily/Google RSS 的英文报道；summary 是正文摘要（Finnhub/Tavily 提供，比 title 信息更全）。**有 summary 时优先基于 summary 判断**。source_tier=finnhub 是专业财经源；google_rss 兜底质量参差要更严格
- **近期报道_按持仓_金十中文**：每只重仓股金十的中文 flash + 文章。这是**中国视角的补充**：
  - 中美关税 / 出口管制 / 供应链 / 监管 角度的解读，英文源经常没有
  - 例如：台积电产能影响、苹果代工新闻、英伟达对华芯片限制
  - flash 是短快讯（实时），文章是深度（intro 是摘要）
- **近期宏观快讯_中文**：金十/财联社/华尔街见闻的全局实时快讯，importance=2（源未标"重要"的普通快讯）：
  - 源标"重要"（importance>=4）的已由 macro-flash 直推，不会重复进这条流
  - 你只看 importance=2 的快讯，判断是否对用户持仓有**间接传导**（如行业政策 / 同业事件）
  - 多数 importance=2 是噪声，**宁可空也不要硬找**——只挑确实有 portfolio 影响的"""
...
def detect_events(db: Session) -> list[dict]:
    ...
    # 金十 per-position 中文搜索（共享一个 MCP session 给 5 只股，省 init 开销）
    jin10_by_symbol: dict[str, dict[str, list[dict]]] = {}
    with jin10_session() as sess:
        if sess is not None:
            for p in stocks:
                jin10_by_symbol[p.symbol] = search_for_position(
                    sess, p.symbol, p.name, flash_limit=5, news_limit=3,
                )

    # 拉中文宏观快讯流（金十+财联社+华尔街见闻），让 LLM 自己判断哪些影响用户持仓
    # importance>=4 的（源标"重要"）由 macro-flash worker 每 3min 直推，不在这里重复处理
    macro_items = [
        m for m in fetch_macro_news(min_importance=2, hours_back=4)
        if m.importance < 4
    ]

    # 任一源有数据都值得调 LLM；都空才跳过
    any_jin10 = any((v["flash"] or v["news"]) for v in jin10_by_symbol.values())
    if (
        all(not v for v in news_by_symbol.values())
        and not macro_items
        and not any_jin10
    ):
        return []
    ...
    payload = {
        ...
        "近期新闻_按持仓_英文源": {
            # Finnhub/Tavily/Google RSS 的英文报道；有 summary 时优先基于 summary 判断
            sym: [
                {
                    "title": n["title"],
        ...
        "近期报道_按持仓_金十中文": {
            # 金十中文报道（flash 快讯 + news 深度文章），覆盖中美关系/芯片政策/供应链等角度
            sym: {
                "快讯": [{"time": f.get("time"), "title": f.get("title") or f.get("content", "")[:80], "content": (f.get("content") or "")[:300]} for f in v["flash"]],
                "文章": [{"time": n.get("time"), "title": n.get("title"), "intro": (n.get("introduction") or "")[:300]} for n in v["news"]],
            }
            for sym, v in jin10_by_symbol.items()
            if v["flash"] or v["news"]
        },
        # 中文宏观快讯（金十/财联社/华尔街见闻）——
        # importance 1-5：源标记的 important/red/score=3 → 4-5；其余 2
        "近期宏观快讯_中文": [
            {
