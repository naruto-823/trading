...
from app.services.briefing import HTTP_HEADERS, fetch_news_for_symbol
...
def detect_events(db: Session) -> list[dict]:
    ...
    with httpx.Client(timeout=10.0, headers=HTTP_HEADERS, follow_redirects=True) as client:
        for p in stocks:
            news_by_symbol[p.symbol] = fetch_news_for_symbol(
                p.symbol, client, name=p.name, limit=NEWS_PER_STOCK,
            )
    ...
    payload = {
        ...
        "近期新闻": {
            sym: [{"title": n["title"], "publisher": n.get("publisher", "")} for n in news]
