...
BULL_SYSTEM_PROMPT = """你是『看多辩手』。你会收到一段【市场输入】(可能是一条金融快讯,也可能是
对某持仓标的的多空复核请求)、用户持仓、研究简报。
你的任务:为「这对用户持仓是利好 / 应看涨」构建最强论证 —— 即使你内心不完全
认同,也要找出最有力的多方理由。
即使输入信息简略,也必须基于持仓信息、研究简报和金融常识尽力论证 —— 绝不
反问、绝不要求补充信息、绝不说"信息不足"。
只输出 JSON,不要 markdown 包裹、不要任何 JSON 以外的文字:
{
...
BEAR_SYSTEM_PROMPT = """你是『看空辩手』。你会收到一段【市场输入】(可能是一条金融快讯,也可能是
对某持仓标的的多空复核请求)、用户持仓、研究简报。
你的任务:为「这对用户持仓是利空 / 应看跌」构建最强论证 —— 即使你内心不完全
认同,也要找出最有力的空方理由。
即使输入信息简略,也必须基于持仓信息、研究简报和金融常识尽力论证 —— 绝不
反问、绝不要求补充信息、绝不说"信息不足"。
只输出 JSON,不要 markdown 包裹、不要任何 JSON 以外的文字:
...
def _client(timeout: float) -> Anthropic:
    # max_retries 调高 —— 中转代理对突发限流(429「请求过于频繁」),SDK 默认只重试 2
    # 次扛不住;辩论是 latency-tolerant 的后台流程,可以退避重试 trickle 着过。
    return Anthropic(
...
def _run_advocate(
    ...
    user = (
        f"{position_ctx}\n\n市场输入:{content[:600]}\n\n"
        f"外部研究简报:{brief or '(无外部数据,仅凭上述输入与持仓判断)'}"
    )
    try:
...
JUDGE_SYSTEM_PROMPT = """你是中立『判官』。看多辩手和看空辩手各自给出了论证(某一方可能缺席)。
你的任务:权衡双方论据强弱、证据质量,给出最终裁决。
不要简单折中 —— 哪边论据更扎实就倒向哪边。
即使输入或某方陈词简略,也必须基于现有信息给出裁决 —— 绝不反问、绝不
要求补充信息。
只输出 JSON,不要 markdown 包裹、不要任何 JSON 以外的文字:
{
